...
int receiveFromGET(char* host, char* resource, char* httpMessage)
{
    ...
    int sockfd = -1;
    char recv_buf[HTTPRECVBUFSIZE];

    memset(&hints, 0, sizeof hints);
    hints.ai_family = AF_UNSPEC;
    hints.ai_socktype = SOCK_STREAM;

    if ((return_error = getaddrinfo(host, "80", &hints, &serverinfo)) != 0) {
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(return_error));
        return 1;
    }
    ...
    // Use strlen here
    ssize_t send_count = writen(sockfd, httpMessage, strlen(httpMessage));
    printf("Send Count: %zi", send_count);



    do{
        received_count = recv(sockfd, recv_buf, HTTPRECVBUFSIZE , 0);
        if(received_count <= 0){
            fprintf(stdout, "%zi bytes received\n", received_count);
        }
        write(read_fd, recv_buf, (size_t) (received_count));
        memset(&recv_buf, 0, sizeof recv_buf);
    }while(received_count == HTTPRECVBUFSIZE);
