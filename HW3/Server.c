            ...
            while(retries <= 10 ) {
                if(readable_timeo(newfd, 1) <= 0) {
                    ...
                    if(retries >= 10) {
                        ...
                        int length = strlen(errorMessage) + sizeof(uint16_t)*2 + 1 ;
                        char buffer[length];                               
                        generateErrorMessage(buffer, errorMessage, 3);
                        sendto(newfd, buffer, length, 0, (struct sockaddr *)&their_addr, addr_len);
			memset(&errorMessage, 0, sizeof errorMessage);
                        memset(&buffer, 0, sizeof buffer);
                        close(newfd);
...
int main(void)
{
    ...
    while(1) {
        ...
        if(parseRRQ(&filename, &mode, buf, numbytes) == 0) {
            ...
            int length = strlen(errorMessage) + sizeof(uint16_t)*2 + 1 ;
            char buffer[length];                               
            generateErrorMessage(buffer, errorMessage, 4);
            sendto(newfd, buffer, length, 0, (struct sockaddr *)&their_addr, addr_len);
            memset(&errorMessage, 0, sizeof errorMessage);
            memset(&buffer, 0, sizeof buffer);
            continue;
        ...
        if (!fork()) { // this is the child process
            ...
            if(isRRQ == 1) {
                if( strcmp(mode, OCTET) == 0 || strcmp(mode, NETASCII) == 0 ) {
                    ...
                    int length = strlen(errorMessage) + sizeof(uint16_t)*2 + 1 ;
                    char buffer[length];                               
                    generateErrorMessage(buffer, errorMessage, 5);
                    sendto(newfd, buffer, length, 0, (struct sockaddr *)&their_addr, addr_len);
                    memset(&errorMessage, 0, sizeof errorMessage);
                    memset(&buffer, 0, sizeof buffer);
                }
                ...
                if (fileSize == -1) {
                    ...
                    char* errorMessage = "Encounter Error while trying to recvfrom."; 
                    int length = strlen(errorMessage) + sizeof(uint16_t)*2 + 1 ;
                    char buffer[length];                               
                    generateErrorMessage(buffer, errorMessage, 6);
                    sendto(newfd, buffer, length, 0, (struct sockaddr *)&their_addr, addr_len);
                    memset(&errorMessage, 0, sizeof errorMessage);
                    memset(&buffer, 0, sizeof buffer);
                } else {
